...
import com.mx.openFeing.MedicoFeing;
import com.mx.openFeing.PacienteFeing;
...
public class ClinicaService {
	...
	public Respuesta guardar(Clinica clinica) {
		...
		if(cliDao.existsById(clinica.getNumero())){
			rs.setMensaje("La clinica no se agrego por que ya existe su numero");
			rs.setSuccess(false);
		...
		if(cliDao.existsById(clinica.getNumero())) {
			for(Clinica c: cliDao.findAll()) {
				if(clinica.getNumero() != c.getNumero() && clinica.getNombre().equalsIgnoreCase(c.getNombre()) ) {
					rs.setMensaje("La clinica no se edito por que su nombre ya existe");
					rs.setSuccess(false);
					rs.setObj(c);
					return rs;
				}
	...
	public Respuesta eliminar(Clinica clinica) {
		...
		if(cliDao.existsById(clinica.getNumero())) {
			...
			if(pacientes != null) {
				rs.setMensaje("La clinica no se peude eliminar por que tiene pacientes");
				rs.setSuccess(false);
				rs.setObj(pacientes);
				return rs;
			}
			...
			if(medicos != null) {
				rs.setMensaje("La clinica no se peude eliminar por que tiene medicos");
				rs.setSuccess(false);
				rs.setObj(medicos);
				return rs;
			}
